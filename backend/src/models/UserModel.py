...
class UserModel(db.Model):
  ...
  def insert(self):
    ...
    VALUES (%s, %s, %s, %s, %s, %s)
    """
    params = (self.name, self.email, self.operator, self.__generate_hash(self.password), self.created_at,self.modified_at)
  ...
  def update(self):
    if self.password == '':
      ...
      WHERE id= %s
      """
      params = (self.__generate_hash(self.password), self.name, self.email, self.operator,datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),self.id)
      return run(query, params)
