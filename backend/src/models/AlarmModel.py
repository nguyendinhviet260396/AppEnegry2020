...
class AlarmModel(db.Model):
  ...
  def insert(self):
    query = """
    INSERT INTO alarmtable (area, code, typealarm, discription, solution,timestamp) 
    VALUES (%s, %s, %s, %s, %s, %s)
    """
    params = (self.area, self.code, self.typealarm, self.discription, self.solution,self.timestamp)
