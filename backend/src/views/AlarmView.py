...
# get realtime data
@alarm_api.route('/add',methods=['POST'])
def add():
  value=request.json
  alarm = AlarmModel()
  alarm.area = value.get('area')
  alarm.code = value.get('code')
  alarm.typealarm = value.get('typealarm')
  alarm.discription = value.get('discription')
  alarm.solution = value.get('solution')
  alarm.insert()
  return custom_response({"state":"true"},200)
# get realtime data
